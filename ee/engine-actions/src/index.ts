		...
		const hook: MasterContainerHook = (builder: MasterContainerBuilder) => {
			return builder
				.setupService('application', (it, { projectContextResolver, debugMode }) => {
					...
					const workerManager = new DispatchWorkerManager(eventDispatcher)
					...
					const actionsWebsocketMiddlewareFactory = new ActionsWebsocketControllerFactory(debugMode, actionsContextResolver, workerManager)
